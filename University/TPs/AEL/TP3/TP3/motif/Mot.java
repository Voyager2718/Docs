...
public class Mot {
	...
	public int indiceMotifNaif(Mot motif) {
		...
		int n = m.length();
		int mf = motif.length();
		for (int i = 0; i <= n - mf; i++) {
			String sub = m.toString().substring(i, i + mf);
			if (motif.toString().equals(sub)) {
				return i;
			}
		}
	...
	public int indiceMotifAutomate(Mot motif) {
		...
		if (motif.length() == 0) {
			...
			for (int i = 0; i <= m.length() - 1; i++) {
				etat = autoMotif.transition(etat, m.toString().charAt(i));
				if (autoMotif.etatFinal() == etat) {
	...
	public int indiceMotifKMP(Mot motif) {
		...
		int q = 0;
		for (int i = 0; i < m.length() - 1; i++) {
			while (q > 0 && motif.toString().charAt(q) != m.charAt(i))
				q = pref[q];
			if (motif.toString().charAt(q) == m.charAt(i))
				q++;
			if (q == motif.length())
